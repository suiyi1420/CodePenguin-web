...
const UserTableList: React.FC = () => {
    ...
    getRoleList().then((res) => {
      if (res.code === 200) {
        ...
        res.rows.map((item: any) => {
          ropt[item.roleId + ''] = item.roleName;
        });
        setRoleOptions(ropt);
      }
    });
  ...
  const columns: ProColumns<UserType>[] = [
    ...
    {
      title: <FormattedMessage id="system.User.role" defaultMessage="用户角色" />,
      dataIndex: 'roleId',
      valueType: 'select',
      valueEnum: roleOptions,
    },
    {
      ...
      <Row gutter={[16, 24]}>
        ...
        <Col lg={18} md={24}>
          <ProTable<UserType>
            ...
            request={(params) =>
              getUserList({ ...params, deptId: selectDept.id, status: '0' } as UserListParams).then(
                (res) => {
                  return {
                    data: res.rows,
                    total: res.total,
